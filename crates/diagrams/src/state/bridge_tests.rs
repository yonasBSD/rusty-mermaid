...
fn layout_composite_has_inner_edges() {
    ...
    assert!(
        result.nodes.iter().any(|n| n.id == "Idle"),
        "should have Idle"
    );
    assert!(
        result.nodes.iter().any(|n| n.id == "Running"),
        "should have Running"
    );
    assert!(
        result.nodes.iter().any(|n| n.id == "Active.[*]_start"),
        "should have inner start"
    );
    ...
    assert!(
        result
            .edges
            .iter()
            .any(|e| e.src == "Active.[*]_start" && e.dst == "Idle"),
    ...
    assert!(
        result
            .edges
            .iter()
            .any(|e| e.src == "Idle" && e.dst == "Running"),
...
fn choice_diamond_edges_clip_to_polygon() {
    ...
    let check_to_a = result
        .edges
        .iter()
        .find(|e| e.src == "check" && e.dst == "A")
        .expect("check → A edge");
    let check_to_b = result
        .edges
        .iter()
...
fn layout_note_right() {
    ...
    assert!(
        note.x > still.x,
        "note (x={:.1}) should be right of Still (x={:.1})",
        note.x,
        still.x
    );
}
...
fn layout_note_left() {
    ...
    assert!(
        note.x < still.x,
        "note (x={:.1}) should be left of Still (x={:.1})",
        note.x,
        still.x
    );
}
...
fn multi_source_end_bullseye_preserves_edges() {
    ...
    assert!(
        (still_edge_start_x - crash_edge_start_x).abs() > 1.0 || (still.x - crash.x).abs() < 1.0, // unless they happen to be x-aligned
...
fn intersect_start_circle_diagonal() {
    ...
    assert!(
        ...
        "point should be on circle boundary, dist={dist}"
    );
}
...
fn intersect_end_bullseye_diagonal() {
    ...
    assert_point_near(
        ...
        Point::new(-s, s),
        1e-6,
        "end bullseye diagonal bottom-left",
    );
}
...
fn intersect_history_circle_diagonal() {
    ...
    assert!(
        approx_eq(dist, r, 1e-6),
        "history circle diagonal point on boundary"
    );
}
...
fn intersect_diamond_diagonal_hits_edge_not_vertex() {
    ...
    assert_point_near(
        p,
        Point::new(hw / 2.0, hw / 2.0),
        1e-6,
        "diamond 45-deg hits edge midpoint",
    );
}
...
fn intersect_with_offset_center() {
    ...
    assert_point_near(
        p,
        Point::new(50.0, 30.0 - r),
        1e-6,
        "offset center from above",
    );
    // Target to the right
    let p = state_shape_intersect(Shape::StateEnd, bbox, Point::new(200.0, 30.0)).unwrap();
    assert_point_near(
        p,
        Point::new(50.0 + r, 30.0),
        1e-6,
        "offset center from right",
    );
}
...
fn intersect_diamond_offset_center() {
    ...
    assert_point_near(
        p,
        Point::new(100.0, 50.0 - hw),
        1e-6,
        "offset diamond from above",
    );
    // Target directly right
    let p = state_shape_intersect(Shape::Choice, bbox, Point::new(300.0, 50.0)).unwrap();
    assert_point_near(
        p,
        Point::new(100.0 + hw, 50.0),
        1e-6,
        "offset diamond from right",
    );
}
