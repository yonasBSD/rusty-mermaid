...
pub fn to_scene_themed(board: &KanbanBoard, theme: &Theme) -> Scene {
    ...
    // Compute card heights and column widths
    ...
    let total_w: f64 = col_widths.iter().sum::<f64>() + (board.columns.len() - 1) as f64 * COLUMN_GAP + MARGIN * 2.0;
    let total_h = max_col_height + MARGIN * 2.0;
    ...
    for (ci, col) in board.columns.iter().enumerate() {
        ...
        for (cardi, card) in col.cards.iter().enumerate() {
            let card_h = col_card_heights[ci][cardi];
            ...
            // Metadata line (priority, assignee, ticket)
            ...
            if !meta_parts.is_empty() {
                scene.push(Primitive::Text {
                    position: rusty_mermaid_core::Point::new(col_cx, card_cy + line_height * 0.5),
