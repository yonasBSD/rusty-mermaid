...
/// After position assignment, compute compound node dimensions from border nodes,
/// then remove all border dummy nodes.
fn remove_border_nodes(g: &mut Graph<NodeLabel, EdgeLabel>) {
    // First pass: compute compound node dimensions from their border nodes
    for nid in g.node_ids().collect::<Vec<_>>() {
        if g.children(nid).next().is_none() {
            continue;
        }
        let node = g.node(nid).unwrap();
        let bt = node.border_top;
        let bb = node.border_bottom;
        let bl = node.border_left.clone();
        let br = node.border_right.clone();

        if let (Some(top_id), Some(bot_id)) = (bt, bb) {
            let t = g.node(top_id).unwrap();
            let b = g.node(bot_id).unwrap();
            let t_y = t.y;
            let b_y = b.y;

            // Get rightmost border left and right
            let max_rank = bl.keys().chain(br.keys()).copied().max().unwrap_or(0);
            if let (Some(&l_id), Some(&r_id)) = (bl.get(&max_rank), br.get(&max_rank)) {
                let l_x = g.node(l_id).unwrap().x;
                let r_x = g.node(r_id).unwrap().x;

                let n = g.node_mut(nid).unwrap();
                n.width = (r_x - l_x).abs();
                n.height = (b_y - t_y).abs();
                n.x = l_x + n.width / 2.0;
                n.y = t_y + n.height / 2.0;
            }
        }
    }
