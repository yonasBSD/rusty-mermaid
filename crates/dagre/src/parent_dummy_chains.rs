        ...
        while let Some(node) = g.node(v) {
            ...
            if ascending {
                while path_idx < path.len() && path[path_idx] != lca {
                    ...
                    let max_rank = g.node(path_node).and_then(|n| n.max_rank).unwrap_or(0);
            ...
            if !ascending {
                while path_idx + 1 < path.len() {
                    ...
                    let min_rank = g.node(next).and_then(|n| n.min_rank).unwrap_or(i32::MAX);
...
fn find_path(
    ...
    let lca = lca.unwrap_or_else(|| *v_path.last().unwrap_or(&v));
