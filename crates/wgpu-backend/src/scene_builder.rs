...
fn append_svg_arc(path: &mut BezPath, rx: f64, ry: f64, rotation: f64, large_arc: bool, sweep: bool, to: &Point) {
    ...
    let svg_arc = kurbo::SvgArc {
        from, to: to_kpoint(to),
        radii: Vec2::new(rx, ry), x_rotation: rotation.to_radians(),
        large_arc, sweep,
    };
